...
void sieve(ll len) {  //generate primes till sqrt(len);
    len = sqrt(len) + 1;
    bs[0] = bs[1] = 1;
    for (ll i = 2; i <= len; i++) if (!bs[i]) {
      for (ll j = i * i; j <= len; j += i) bs[j] = true;
      primes.PB(i);
    }
}
...
vi factorize(ll N) {
    ...
    ll i = 0, p = primes[i]; // using primes = 2, 3, 5, ...
    while (N != 1 && (p * p <= N)) { // stop at sqrt(N), but N can get smaller
        while (N % p == 0) { N /= p; factors.PB(p); } // remove this PF
        p = primes[++i]; // only consider primes!
    }
    if (N != 1) factors.PB(N); // special case if N is actually a prime
    return factors;
}
